...
avec quelques paquets spécifiques pour ensuite l'utiliser avec `Docker` et `Singularity`.
 ...
 - Utilisez docker singularity sous biosphere
 - Trouvez la bonne image biosphere contenant `Docker`
...
`Docker` ne fonctionne pas sur le cluster. Il faut le faire soit sur une machine locale
soit sur une VM de l'IFB ([Biosphère](https://biosphere.france-bioinformatique.fr/)).
...
le fichier `canevaDockerfile` pour avoir dans l'image : la dernieèère 
...
4 - Verifiez que `Singularity` est bien installés sur vos machines:
```
$ singularity --version
```
