...
On va ensuite récupérer l'image `Docker` officielle de `R` sur le dépot de `rstudio`
...
Une fois le fichier complété il ne reste plus qu'a construire l'image `Docker`
avec :
...
de progammes / dépendances à installer. Cependant cette étape est parallélisée, 
...
Pour supprimmer une image 
`Docker` il suffit de faire :
...
-   Télécharger une image existante depuis le dépot `Singularity` : [Sylabs Hub](https://cloud.sylabs.io/library) 
(remplace Singularity Hub qui n'est plus supporté depuis fin 2021)
...
C'est une des deux façons les plus simples d'obtenir une image `Singularity`.
On commence par vérifier que la bonne version de `Singularity` est installée :
...
que l'on souhaite utiliser et d'utiliser `Singularity` pour construire la nouvelle image.
