...
Ici nous allons voir comment créer une image docker avec quelques paquets spécifiques pour ensuite l'utiliser avec `Docker` et `Singularity`.
...
installant les librairies Linux et les outils nécessaires aux analyses souhaitées.
Il s'agit surtout d'un script de lignes de commandes compilé dans un format spécifique.

`Docker` ne fonctionne pas sur un cluster de calcul dont l'administration système est gérée par un informaticien.
Il faut l'utiliser soit sur une machine locale soit sur une machine virtuelle (VM) du Cloud Biosphère de l'IFB.
